...
UserCollection.prototype.register = function (data) {
  ...
  .then(function () {
    return createUser(data, self);
  })
  .then(function (user) {
    if (self.app.multitenant) {
      return self.createUserDb(user)
      .then(function () {
        user.dbName = 'user_' + user._id;
        return user;
      });
    }
    else {
...
UserCollection.prototype.createUserDb = function (user) {
  var dbName = 'user_' + user._id;
  return this.db.createDb(dbName);
};
