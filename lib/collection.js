...
Collection.prototype.setup = function () {
  ...
  if (this.collection.isUserType) {
    ...
    newDoc.views.byTwitterId = {
      map: "function (doc) { if (doc.collection == '" + name + "' && doc.twitterId) { emit(doc.twitterId, doc); } }"
    };
    newDoc.views.byFacebookId = {
      map: "function (doc) { if (doc.collection == '" + name + "' && doc.facebookId) { emit(doc.facebookId, doc); } }"
    };
  }
