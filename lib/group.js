...
GroupCollection.prototype.contains = function (id, opts) {
  ...
  .then(function (results) {
    if (opts && (opts.includeGroups || opts.include_docs || opts.includeDocs)) {
      var groupIds = _.uniq(_.pluck(results, 1));
...
GroupCollection.design.views = {
  contains: {
    map: function (doc) { 
      if (doc.collection == '{{name}}' && doc.users) { 
        for (var i=0, l=doc.users.length; i<l; i++) { 
  ...
  byGroupName: {
    map: function (doc) {
      if (doc.collection == '{{name}}' && doc.name) {
        emit(doc.name, doc);
      }
    }
  }
...
GroupCollection.design.updates = {
  addUser: function (doc, req) {
    ...
    if (doc.users) {
      doc.users = doc.users.filter(function (userId) {
        return userId !== user;
      });
