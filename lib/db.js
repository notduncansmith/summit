...
Database.prototype.destroy = function(name) {
  ...
  .get('_rev')
  .then(function (rev) {
    return self.nano.destroyAsync(name);
  });
};
...
Database.prototype.create = function(name) {
  ...
  if (_.isArray(name)) {
    return Promise.all(name.map(this.create.bind(this)));
  }

  return this.nano.createAsync(name);
};
...
Database.prototype.getAttachment = function (docId, attId) {
  return this.attachments.getAsync(docId, attId);
};
