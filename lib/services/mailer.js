...
function Mailer (config, coll) {
  this.config = config;
  this.coll = coll;
  if (this.config.mailer.transportOptions.transport === 'mailgun'){
    this.client = nodemailer.createTransport(mg(this.config.mailer.transportOptions));  
  }else{
    this.client = nodemailer.createTransport(this.config.mailer.transportOptions);
  }
}

Mailer.prototype.send = function (templateName, toUser, withData, fromEmail, fromName) {
  var fromEmail = fromEmail || this.config.mailer.fromEmail;
  var fromName = fromName || this.config.mailer.fromName;
  var toEmail = ''
    , user = {}
    , withData = withData || {}
    , client = this.client;
...
function sendMail (opts) {
  ...
  return new Promise(function (resolve, reject) {
    client.sendMail(message, function (result) {
      resolve(result);
    });
  });
}