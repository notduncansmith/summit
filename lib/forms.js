...
function addValues (formString, values) {
  ...
  if (!_.isUndefined(values._attachments)) {
    $('.form-group')
    .first()
    .prepend('<input type="hidden" name="_attachments">')
  }
  ...
  $('input[name="_attachments"]').val(JSON.stringify(values._attachments));
