...
Database.prototype.use = function (name) {
  ...
  if (name.indexOf('http://') === 0) {
    host = name.substring(0, name.indexOf('/', 7));
    name = name.substring(name.indexOf('/', 7));
  }
  else if (name.indexOf('https://') === 0) {
    host = name.substring(0, name.indexOf('/', 8));
    name = name.substring(name.indexOf('/', 8));
  }
  var config = _.extend({}, this.config, {name: name, connString: host});
