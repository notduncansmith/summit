  ...
  var EmailTemplate = app.collection({
    ...
    design: {
      views: {
        byName: {
          map: function (doc) {
            if (doc.collection == 'EmailTemplate' && doc.name) {
              emit(doc.name, doc);
            }
          }
        }
...
Mailer.prototype.send = function(template, toUser, withData, fromEmail, fromName) {
  ...
  return this._render(template, withData)
