...
function Summit (userConfig) {
  ...
  this.use(bogart.middleware.flash());
  this.use(bogart.middleware.methodOverride());
...
Summit.prototype._setup = function () {
  ...
  this.use(bogart.middleware.directory(path.join(__dirname, 'cms/assets')));
...
Summit.prototype._startWithSSL = function (env) {
  var keyFileContents = fs.readFileSync(env.ssl.key);
  var certFileContents = fs.readFileSync(env.ssl.cert);

  var ssl = {key: keyFileContents, cert: certFileContents};

  self.app.start(env.ssl);
  ...
  if (env.ssl.forceSsl) {
    ...
    rRouter.get(/(.*)/, function(req){
      var port = req.port === 443 ? "" : req.port;
      return bogart.redirect("https://" + req.host +  req.params.splat[0])
    });
