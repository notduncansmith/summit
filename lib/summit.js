...
function Summit (userConfig) {
  ...
  if (this.env.ssl) {
    this.app = getSSLApp(this.env.ssl);
  }
  else {
    this.app = bogart.app();
  }
...
function getSSLApp (sslOpts) {
  var keyFileContents = fs.readFileSync(sslOpts.key);
  var certFileContents = fs.readFileSync(sslOpts.cert);

  return bogart.app({ssl: {key: myKeyFileContents, cert: myCertFileContents}});
}
