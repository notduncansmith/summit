...
Summit.prototype._setup = function () {
  ...
  .onCreateLocals(function (user, app, locals) {
    ...
    return _.extend({}, app.templateGlobals, defaultLocals, locals);
  });
  ...
  // an API perspective.
  // If I want to include middleware that's
  // not conceptually a router, but I want
  // to render a view (say, unauthorized) I'd
  // normally have to push it onto the router array
  // myself so it gets used in the proper order.
